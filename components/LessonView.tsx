...
import { Volume2, ArrowLeft, CheckCircle2, XCircle, Sparkles, HelpCircle, Award, Play, RotateCcw, Heart, RefreshCw, BookOpen, Zap, ArrowRight, Languages, Mic, MicOff, Square, AlertCircle, ThumbsUp, Radio, Flag, Download, FileText, Printer, Check } from 'lucide-react';
...
/* --- Lesson Offline Summary Generators --- */
export const generateTextSummary = (day: DayLesson): string => {
  let content = `====================================================\n`;
  content += `OFFLINE LESSON SUMMARY & STUDY GUIDE\n`;
  content += `Day ${day.dayNumber}: ${day.title}\n`;
  content += `Category: ${day.category} | Level: ${day.level}\n`;
  content += `====================================================\n\n`;

  content += `--- LESSON OVERVIEW ---\n`;
  content += `${day.theory.summary}\n\n`;

  if (day.theory.rules && day.theory.rules.length > 0) {
    content += `--- KEY GRAMMAR RULES & EXPLANATIONS ---\n`;
    day.theory.rules.forEach((r, idx) => {
      content += `Rule ${idx + 1}: ${r.rule}\n`;
      content += `  Why: ${r.why}\n`;
      content += `  [+] Example: "${r.example}"\n`;
      if (r.wrongExample) {
        content += `  [-] Common Mistake: "${r.wrongExample}"\n`;
      }
      content += `\n`;
    });
  }

  if (day.theory.wordExplanations && day.theory.wordExplanations.length > 0) {
    content += `--- WORD-BY-WORD PURPOSE BREAKDOWN ---\n`;
    day.theory.wordExplanations.forEach((w) => {
      content += `- ${w.word} (${w.role}): ${w.whyUsed}\n`;
    });
    content += `\n`;
  }

  if (day.theory.proTip) {
    content += `--- PRO TIP ---\n`;
    content += `💡 ${day.theory.proTip}\n\n`;
  }

  if (day.quiz && day.quiz.length > 0) {
    content += `--- PRACTICE QUIZ & REVISION ---\n`;
    day.quiz.forEach((q, idx) => {
      content += `Q${idx + 1}: ${q.question}\n`;
      q.options.forEach((opt, oIdx) => {
        const isCorrect = oIdx === q.correctAnswerIndex;
        content += `   ${String.fromCharCode(65 + oIdx)}) ${opt}${isCorrect ? '  [CORRECT]' : ''}\n`;
      });
      content += `   Explanation: ${q.explanationWhy}\n\n`;
    });
  }

  content += `====================================================\n`;
  content += `Generated by LingoSpark - Master English with Confidence\n`;
  content += `====================================================\n`;

  return content;
};

export const downloadTextSummary = (day: DayLesson) => {
  const text = generateTextSummary(day);
  const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `Day_${day.dayNumber}_${day.title.replace(/[^a-zA-Z0-9]/g, '_')}_Summary.txt`;
...
export const printPdfSummary = (day: DayLesson) => {
  const printWindow = window.open('', '_blank');
  if (!printWindow) {
    downloadTextSummary(day);
    return;
  }
  ...
  const html = `
    ...
    <html>
      <head>
        <title>Day ${day.dayNumber} - ${day.title} Offline Study Guide</title>
        <meta charset="utf-8">
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; padding: 32px; color: #0f172a; max-width: 800px; margin: 0 auto; line-height: 1.6; background: #ffffff; }
          .header { border-bottom: 2px solid #6366f1; padding-bottom: 12px; margin-bottom: 20px; }
          h1 { color: #312e81; font-size: 26px; margin: 0 0 4px 0; }
          .subtitle { color: #64748b; font-size: 13px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.05em; }
          .summary-box { background: #f8fafc; border-left: 4px solid #6366f1; padding: 14px 18px; margin-bottom: 24px; border-radius: 0 8px 8px 0; font-size: 15px; }
          .section-title { color: #1e1b4b; font-size: 17px; font-weight: 800; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px; margin-top: 28px; margin-bottom: 14px; }
          .rule-card { background: #f1f5f9; border-radius: 10px; padding: 14px 18px; margin-bottom: 14px; page-break-inside: avoid; }
          .rule-title { font-weight: 800; color: #1e1b4b; font-size: 15px; margin-bottom: 6px; }
          .why-box { background: #fef3c7; border: 1px solid #fde68a; color: #92400e; padding: 10px 14px; border-radius: 8px; font-size: 13px; margin: 8px 0; }
          .example { color: #047857; font-weight: 600; font-size: 13.5px; margin-top: 4px; }
          .wrong-example { color: #b91c1c; font-style: italic; font-size: 13px; margin-top: 2px; }
          .protip-box { background: #e0e7ff; border: 1px solid #c7d2fe; border-radius: 8px; padding: 12px 16px; margin-bottom: 14px; font-size: 13.5px; color: #3730a3; }
          .quiz-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 16px; margin-bottom: 12px; font-size: 13.5px; page-break-inside: avoid; }
          .correct-opt { color: #047857; font-weight: bold; }
      ...
      <body>
        <div class="header">
          ...
          <div class="subtitle">Category: ${day.category} • Target Level: ${day.level}</div>
        </div>
        
        <div class="summary-box">
          <strong>Lesson Overview:</strong><br/>
          ${day.theory.summary}
        </div>

        ${day.theory.rules && day.theory.rules.length > 0 ? `
          <div class="section-title">Key Grammar Rules & Explanations</div>
          ${day.theory.rules.map((r, i) => `
            <div class="rule-card">
              <div class="rule-title">${i + 1}. ${r.rule}</div>
              <div class="why-box"><strong>💡 Why this rule works:</strong> ${r.why}</div>
              <div class="example">✅ Example: "${r.example}"</div>
              ${r.wrongExample ? `<div class="wrong-example">❌ Common Mistake: "${r.wrongExample}"</div>` : ''}
            </div>
          `).join('')}
        ` : ''}
        ...
        ${day.theory.proTip ? `
          ...
          <div class="protip-box">
            💡 <strong>Grammar Pro Tip:</strong> ${day.theory.proTip}
          </div>
        ` : ''}

        ${day.quiz && day.quiz.length > 0 ? `
          <div class="section-title">Practice Quiz & Self-Review</div>
          ${day.quiz.map((q, i) => `
            <div class="quiz-card">
              <strong>Q${i + 1}: ${q.question}</strong>
              <ul style="margin: 6px 0; padding-left: 20px;">
                ${q.options.map((opt, oIdx) => `
                  <li class="${oIdx === q.correctAnswerIndex ? 'correct-opt' : ''}">
                    ${opt} ${oIdx === q.correctAnswerIndex ? '✓ (Correct)' : ''}
                  </li>
                `).join('')}
              </ul>
              <div style="color: #475569; font-size: 12px; font-style: italic;">Explanation: ${q.explanationWhy}</div>
            </div>
          `).join('')}
        ` : ''}

        <div class="footer">
          Generated from LingoSpark English App • Master Rules Offline
        </div>
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          {/* Offline Lesson Summary Download Dropdown */}
          <div className="relative">
            ...
            {showDownloadMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-2 z-50 space-y-1 animate-fadeIn">
                <div className="px-3 py-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-800">
                  Offline Lesson Summary
                </div>
                <button
                  onClick={() => {
                    printPdfSummary(day);
                  ...
                  className="w-full text-left px-3 py-2 rounded-xl text-xs font-medium text-slate-200 hover:bg-indigo-600/30 hover:text-indigo-200 flex items-center space-x-2 transition"
                >
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-xl text-xs font-medium text-slate-200 hover:bg-emerald-600/30 hover:text-emerald-200 flex items-center space-x-2 transition"
                >
                  <FileText className="w-4 h-4 text-emerald-400" />
                  <span>Download Text Guide (.txt)</span>
                </button>
              </div>
            )}
          </div>
      ...
      {step === 'theory' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-2xl p-4 border border-emerald-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-lg">
            <div className="flex items-center space-x-3">
              <div className="p-2.5 bg-emerald-500/20 rounded-xl border border-emerald-500/30 text-emerald-400 shrink-0">
                <Download className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-xs font-bold text-white flex items-center space-x-1.5">
                  <span>Download Lesson Summary for Offline Review</span>
                  <span className="text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full uppercase tracking-wider">PDF & TXT</span>
                </h4>
                <p className="text-[11px] text-slate-300">Save complete rules, explanations, and practice questions to study anytime offline.</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 shrink-0">
              <button
                onClick={() => printPdfSummary(day)}
                className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold flex items-center space-x-1.5 transition shadow-md"
              >
                <Printer className="w-3.5 h-3.5" />
                <span>Save PDF</span>
              </button>
              <button
                onClick={() => downloadTextSummary(day)}
                className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center space-x-1.5 transition shadow-md"
              >
                <FileText className="w-3.5 h-3.5" />
                <span>Text Summary (.txt)</span>
              </button>
            </div>
          </div>
      ...
      {step === 'completed' && (
        <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800 text-center space-y-6 shadow-2xl animate-fadeIn">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <button
              onClick={() => printPdfSummary(day)}
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-6 py-3 rounded-2xl shadow-lg transition text-xs flex items-center space-x-2"
            >
              <Printer className="w-4 h-4" />
              <span>Download Offline PDF Study Sheet</span>
            </button>

            <button
              onClick={onBack}
              className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-8 py-3 rounded-2xl shadow-lg transition text-xs"
            >

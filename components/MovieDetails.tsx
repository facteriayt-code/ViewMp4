    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 bg-black/90 backdrop-blur-md overflow-y-auto modal-scroll-container">
      <div className="relative bg-[#141414] w-full max-w-3xl min-h-screen md:min-h-0 md:rounded-2xl overflow-hidden shadow-2xl border border-white/5 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="relative aspect-video group">
          ...
          {/* Main Play Button - Always visible for ease of use */}
          <div className="absolute inset-0 flex items-center justify-center">
             <button 
              onClick={() => onPlay(movie)}
              className="bg-red-600 text-white w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(229,9,20,0.5)] hover:scale-110 active:scale-95 transition-all duration-300 border-4 border-white/20"
             >
                <Play className="w-10 h-10 md:w-12 md:h-12 fill-white ml-2" />
             </button>
          </div>
        </div>
        ...
        <div className="p-6 md:p-8 space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-4 text-xs font-bold text-gray-500 uppercase tracking-widest">
              ...
              <span className="flex items-center">
                <Eye className="w-3 h-3 mr-1" /> {formatViews(movie.views)} views
              </span>
            </div>
          </div>
          ...
          {/* 3. Play & Share Buttons */}
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button 
              onClick={() => onPlay(movie)}
              className="flex-1 bg-white text-black py-4 rounded-xl flex items-center justify-center font-black uppercase tracking-widest hover:bg-gray-200 transition-all active:scale-95 shadow-lg"
            >
            ...
            <button 
              onClick={handleShare}
              className={`flex-1 py-4 rounded-xl flex items-center justify-center font-black uppercase tracking-widest transition-all active:scale-95 border-2 ${copied ? 'bg-green-600 border-green-600 text-white' : 'bg-transparent border-white/10 text-white hover:bg-white/5'}`}
            >
          ...
          <div className="pt-8 border-t border-white/5 space-y-4">
            <div className="flex items-center space-x-2 text-gray-100">
              ...
              <h3 className="text-lg font-black uppercase tracking-tighter">Recommended for you</h3>
            </div>
